...
namespace picoio {
   ...
   /**
    * @defgroup picoiotoken pico.token
    * @ingroup picoiocontracts
    *
    * pico.token contract
    *
    * @details pico.token contract defines the structures and actions that allow users to create, issue, and manage
    * tokens on picoio based blockchains.
    * @{
    */
   class [[picoio::contract("pico.token")]] token : public contract {
      public:
          ...
          *
          * More information can be read [here](https://github.com/PICOIO/picoio.contracts/issues/62)
          * and [here](https://github.com/PICOIO/picoio.contracts/issues/61).
          */
         [[picoio::action]]
         ...
         using create_action = picoio::action_wrapper<"create"_n, &token::create>;
         using issue_action = picoio::action_wrapper<"issue"_n, &token::issue>;
         using retire_action = picoio::action_wrapper<"retire"_n, &token::retire>;
         using transfer_action = picoio::action_wrapper<"transfer"_n, &token::transfer>;
         using open_action = picoio::action_wrapper<"open"_n, &token::open>;
         using close_action = picoio::action_wrapper<"close"_n, &token::close>;
      private:
         struct [[picoio::table]] account {
            asset    balance;

            uint64_t primary_key()const { return balance.symbol.code().raw(); }
         };

         struct [[picoio::table]] currency_stats {
            asset    supply;
            asset    max_supply;
            name     issuer;
         ...
         typedef picoio::multi_index< "accounts"_n, account > accounts;
         typedef picoio::multi_index< "stat"_n, currency_stats > stats;

         void sub_balance( const name& owner, const asset& value );
         void add_balance( const name& owner, const asset& value, const name& ram_payer );
   };
   /** @}*/ // end of @defgroup picoiotoken pico.token
} /// namespace picoio
