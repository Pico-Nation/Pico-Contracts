...
#include <picoio/multi_index.hpp>
#include <picoio/permission.hpp>
#include <picoio/privileged.hpp>
#include <picoio/serialize.hpp>
...
namespace picoiosystem {
   ...
   picoio::block_signing_authority system_contract::convert_to_block_signing_authority( const picoio::public_key& producer_key ) {
      return picoio::block_signing_authority_v0{ .threshold = 1, .keys = {{producer_key, 1}} };
   }

   void system_contract::register_producer( const name& producer, const picoio::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      auto prod = _producers.find( producer.value );
      const auto ct = current_time_point();

      picoio::public_key producer_key{};
      std::visit( [&](auto&& auth ) {
         if( auth.keys.size() == 1 ) {
      ...
      if ( prod != _producers.end() ) {
         ...
         _producers.emplace( producer, [&]( producer_info& info ){
            ...
            info.location        = location;
            info.last_claim_time = ct;
            info.last_block_time = time_point(picoio::seconds(0));
            info.top21_chosen_time = time_point(picoio::seconds(0));
            info.punished_until  = time_point(picoio::seconds(0));
            info.last_expected_produced_blocks_update = ct;
            info.producer_authority.emplace( producer_authority );
         });
         _producers2.emplace( producer, [&]( producer_info2& info ){
   ...
   void system_contract::regproducer( const name& producer, const picoio::public_key& producer_key, const std::string& url, uint16_t location ) {
      check( url.size() < 512, "url too long" );
      check( producer_key != picoio::public_key(), "public key should not be the default value" );
      require_auth( producer );

      register_producer( producer, convert_to_block_signing_authority( producer_key ), url, location );
   }

   void system_contract::regproducer2( const name& producer, const picoio::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      require_auth( producer );
      check( url.size() < 512, "url too long" );
   ...
   void system_contract::update_elected_producers( const block_timestamp& block_time ) {
      ...
      std::sort( producers.begin(), producers.end(), []( const picoio::producer_authority& lhs, const picoio::producer_authority& rhs ) {
   ...
   double system_contract::stake2vote( int64_t staked, time_point locked_stake_period ) const {
      ...
      const auto weeks_to_mature = fmax( ((locked_stake_period - current_time_point())).count() / picoio::days(7).count(), 0 );
      const auto pico_weight = 1.0 - (weeks_to_mature * 7) / (_gpicostate.stake_lock_period.count() / picoio::days(1).count());
      
      const double pico_weight = std::pow( 2, int64_t((current_time_point().sec_since_epoch() - (block_timestamp::block_timestamp_epoch / 1000)) / (seconds_per_day * 7)) / double(52) );

      const auto vote_weight = double(staked) * pico_weight * pico_weight;
   ...
   void system_contract::voteproducer( const name& voter_name, const name& proxy, const std::vector<name>& producers ) {
      ...
      if( rex_itr != _rexbalance.end() && rex_itr->rex_balance.amount > 0 ) {
         check_voting_requipicoent( voter_name, "voter holding REX tokens must vote for at least 21 producers or for a proxy" );
      }
   }
