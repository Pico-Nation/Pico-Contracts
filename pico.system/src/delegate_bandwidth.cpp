...
// Unfortunately, this is needed until CDT fixes the duplicate symbol error with picoio::send_deferred
...
namespace picoiosystem {
   ...
   struct [[picoio::table, picoio::contract("pico.system")]] refund_request {
      name            owner;
      time_point_sec  request_time;
      time_point      last_claim_time;
      time_point      unlock_time;
      picoio::asset    resource_amount;

      bool is_empty()const { return resource_amount.amount == 0; }
      uint64_t  primary_key()const { return owner.value; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      PICOLIB_SERIALIZE( refund_request, (owner)(request_time)(last_claim_time)(unlock_time)(resource_amount) )
   };
    ...
    */
   typedef picoio::multi_index< "refunds"_n, refund_request >      refunds_table;
                                   ...
                                   const asset& stake_delta, bool transfer )
   {
      ...
      int64_t discount = 0;
      if ( picoio::attribute::has_attribute( _gpicostate.gifter_attr_contract, _gpicostate.gifter_attr_issuer, source_stake_from, _gpicostate.gifter_attr_name ) ) {
         discount = picoio::attribute::get_attribute<int64_t>(_gpicostate.gifter_attr_contract, _gpicostate.gifter_attr_issuer, source_stake_from, _gpicostate.gifter_attr_name);
                                     ...
                                     bool transfer )
   {
      ...
      _voters.modify( voter, same_payer, [&]( auto& v ) {
         ...
         v.stake_lock_time = ct
               ...
               + microseconds{ static_cast< int64_t >( restake_rate * _gpicostate.stake_lock_period.count() ) };
      });

      // transfer staked tokens to stake_account (pico.stake)
      // for pico.stake both transfer and refund make no sense
                                       ...
                                       const asset& unstake_quantity)
   {
      ...
      // for picoio.stake both transfer and refund make no sense
      if ( stake_account != from ) {
         ...
         if ( req != refunds_tbl.end() ) { //need to update refund
            ...
            refunds_tbl.emplace( from, [&]( refund_request& r ) {
               ...
               r.unlock_time        = current_time_point() + _gpicostate.stake_unlock_period;
            });
   ...
   void system_contract::refund( const name& owner ) {
      ...
      const auto ct = current_time_point();
      check( ct - req.last_claim_time > picoio::days( 1 ), "already claimed refunds within past day" );
