...
#include <pico.system/rex.results.hpp>
...
namespace picoiosystem {
   ...
   void system_contract::buyrex( const name& from, const asset& amount )
   {
      ...
      check_voting_requipicoent( from );
      ...
      rex_results::buyresult_action buyrex_act( rex_account, std::vector<picoio::permission_level>{ } );
   ...
   void system_contract::unstaketorex( const name& owner, const name& receiver, const asset& from_net, const asset& from_cpu )
   {
      ...
      check_voting_requipicoent( owner );
   ...
   void system_contract::sellrex( const name& from, const asset& rex )
   {
      ...
      if ( current_order.success ) {
         rex_results::sellresult_action sellrex_act( rex_account, std::vector<picoio::permission_level>{ } );
   ...
   void system_contract::closerex( const name& owner )
   {
      ...
      /// check for picoaining rex balance
      {
         auto rex_itr = _rexbalance.find( owner.value );
         if ( rex_itr != _rexbalance.end() ) {
            check( rex_itr->rex_balance.amount == 0, "account has picoaining REX balance, must sell first");
   ...
   /**
    * @brief Checks if account satisfies voting requipicoent (voting for a proxy or 21 producers)
    * for buying REX
    *
    * @param owner - account buying or already holding REX tokens
    * @err_msg - error message
    */
   void system_contract::check_voting_requipicoent( const name& owner, const char* error_msg )const
   {
   ...
   void system_contract::add_loan_to_rex_pool( const asset& payment, int64_t rented_tokens, bool new_loan )
   {
      _rexpool.modify( _rexpool.begin(), same_payer, [&]( auto& rt ) {
         ...
         // incpicoent loan_num if a new loan is being created
         if ( new_loan ) {
            rt.loan_num++;
         }
      });
    ...
    */
   void system_contract::picoove_loan_from_rex_pool( const rex_loan& loan )
   {
   ...
   int64_t system_contract::update_renewed_loan( Index& idx, const Iterator& itr, int64_t rented_tokens )
   {
      ...
      idx.modify ( itr, same_payer, [&]( auto& loan ) {
         ...
         loan.expiration         += picoio::days(30);
   ...
   void system_contract::runrex( uint16_t max )
   {
      ...
      auto process_expired_loan = [&]( auto& idx, const auto& itr ) -> std::pair<bool, int64_t> {
         ...
         picoove_loan_from_rex_pool( *itr );
      ...
      /// transfer from pico.names to pico.rex
         ...
         for ( uint16_t i = 0; i < max; ++i ) {
            ...
            if ( bitr != _rexbalance.end() ) { // should always be true
               ...
               if ( result.success ) {
                  ...
                  rex_results::orderresult_action order_act( rex_account, std::vector<picoio::permission_level>{ } );
   ...
   int64_t system_contract::rent_rex( T& table, const name& from, const name& receiver, const asset& payment, const asset& fund )
   {
      ...
      table.emplace( from, [&]( auto& c ) {
         ...
         c.expiration   = current_time_point() + picoio::days(30);
         c.loan_num     = pool->loan_num;
      });

      rex_results::rentresult_action rentresult_act{ rex_account, std::vector<picoio::permission_level>{ } };
   ...
   void system_contract::channel_to_rex( const name& from, const asset& amount )
   {
      ...
      if ( rex_available() ) {
         ...
         transfer_act.send( from, rex_account, amount,
                            std::string("transfer from ") + from.to_string() + " to pico.rex" );
      }
#endif
   }
   ...
   {
      /**
       * If CORE_SYMBOL is (PICO,4), maximum supply is 10^10 tokens (10 billion tokens), i.e., maximum amount
   ...
   /**
    * @brief Reads amount of REX in savings bucket and picooves the bucket from maturities
    *
    * Reads and (temporarily) picooves REX savings bucket from REX maturities in order to
    * allow uniform processing of picoaining buckets as savings is a special case. This
