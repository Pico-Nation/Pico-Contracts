...
namespace picoio {
   /**
    * @defgroup picoiomsig pico.msig
    * @ingroup picoiocontracts
    * pico.msig contract defines the structures and actions needed to manage the proposals and approvals on blockchain.
    * @{
    */
   class [[picoio::contract("pico.msig")]] multisig : public contract {
      public:
         ...
         void approve( name proposer, name proposal_name, permission_level level,
                       const picoio::binary_extension<picoio::checksum256>& proposal_hash );
         /**
          * Revoke proposal
          *
          ...
          * only after time has expired on the proposed transaction. It picooves corresponding entries from
         ...
         using propose_action = picoio::action_wrapper<"propose"_n, &multisig::propose>;
         using approve_action = picoio::action_wrapper<"approve"_n, &multisig::approve>;
         using unapprove_action = picoio::action_wrapper<"unapprove"_n, &multisig::unapprove>;
         using cancel_action = picoio::action_wrapper<"cancel"_n, &multisig::cancel>;
         using exec_action = picoio::action_wrapper<"exec"_n, &multisig::exec>;
         using invalidate_action = picoio::action_wrapper<"invalidate"_n, &multisig::invalidate>;

      private:
         struct [[picoio::table]] proposal {
            name                            proposal_name;
         ...
         typedef picoio::multi_index< "proposal"_n, proposal > proposals;

         struct [[picoio::table]] old_approvals_info {
            name                            proposal_name;
         ...
         typedef picoio::multi_index< "approvals"_n, old_approvals_info > old_approvals;
         ...
         struct [[picoio::table]] approvals_info {
            uint8_t                 version = 1;
            name                    proposal_name;
         ...
         typedef picoio::multi_index< "approvals2"_n, approvals_info > approvals;

         struct [[picoio::table]] invalidation {
            name         account;
         ...
         typedef picoio::multi_index< "invals"_n, invalidation > invalidations;
   };
   /** @}*/ // end of @defgroup picoiomsig pico.msig
} /// namespace picoio
