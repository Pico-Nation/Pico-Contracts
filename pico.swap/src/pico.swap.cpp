...
namespace picoio {
   ...
   {
      require_auth( get_self() );
      check(in_swap_min_amount > 0, "the minimum amount to swap tokens in picochain should be a positive");
      check(out_swap_min_amount > 0, "the minimum amount to swap tokens from picochain should be a positive");
   ...
   {
      const picoiosystem::key_weight owner_pubkey_weight{
         .key = owner_key,
         .weight = 1,
      };
      const picoiosystem::key_weight active_pubkey_weight{
         .key = active_key,
         .weight = 1,
      };
      const picoiosystem::authority owner{
         .threshold = 1,
         .keys = {owner_pubkey_weight},
         .accounts = {},
         .waits = {}
      };
      const picoiosystem::authority active{
         .threshold = 1,
      ...
      picoiosystem::system_contract::newaccount_action newaccount(system_account, {get_self(), system_contract::active_permission});
      picoiosystem::system_contract::delegatebw_action delegatebw(system_account, {get_self(), system_contract::active_permission});
   ...
   {
      picoiosystem::system_contract::torewards_action torewards(system_account, {get_self(), system_contract::active_permission});
