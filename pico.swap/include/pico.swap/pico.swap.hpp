...
namespace picoio {
   ...
   /**
    * @defgroup picoswap pico.swap
    * @ingroup picoiocontracts
    *
    * pico.swap contract
    *
    * @details pico.swap contract defines the structures and actions that allow users to init token swap,
    * finish token swap, create new account and cancel token swap.
    * @{
    */
   class [[picoio::contract("pico.swap")]] swap : public contract {
   public:
       ...
       *
       * @details Initiate token swap in picochain.
       *
       ...
       *
       * @param chain_id - the chain identifier of the picochain,
       ...
       * @param in_swap_min_amount - the minimum amount to swap tokens from chain_id to picochain,
       * @param out_swap_min_amount - the minimum amount to swap tokens from picochain to chain_id.
       */
      [[picoio::action]]
   ...
   private:
      ...
      struct [[picoio::table]] swap_data {
         ...
         PICOLIB_SERIALIZE( swap_data, (key)(txid)(swap_id)(swap_timestamp)
                                      (status)(provided_approvals)
         )
      };

      struct [[picoio::table]] swapparams {
         string    chain_id;
         string    eth_swap_contract_address;
         string    eth_return_chainid;

         // explicit serialization macro is not necessary, used here only to improve compilation time
         PICOLIB_SERIALIZE( swapparams, (chain_id)(eth_swap_contract_address)(eth_return_chainid) )
      };

      struct [[picoio::table]] chains {
         name     chain;
         bool     input;
         bool     output;
         int64_t  in_swap_min_amount  = 1000000; // minimum amount to swap tokens in picochain
         int64_t  out_swap_min_amount = 5000000; // minimum amount to swap tokens from picochain

         uint64_t primary_key() const { return chain.value; }

         // explicit serialization macro is not necessary, used here only to improve compilation time
         PICOLIB_SERIALIZE( chains, (chain)(input)(output)(in_swap_min_amount)(out_swap_min_amount) )
      };
   ...
   /** @}*/ // end of @defgroup picoswap pico.swap
   ...
   inline public_key string_to_public_key(std::string_view s) {
      ...
      bool is_k1_type = s.size()   >= 3 && ( s.substr(0, 3) == "PICO" || s.substr(0, 3) == "REM");
